...
try:
    ...
    if not confirmacao_wf:
        print("Workflow de confirmacao nao encontrado!")
        exit()
        
    print('WF Achado:', confirmacao_wf['name'], 'ID:', confirmacao_wf['id'])
    ...
    for idx, e in enumerate(execs):
        print(f"{idx+1}. ID: {e['id']} | Status: {e['status']} | Inicio: {e['startedAt']}")
    ...
    if len(execs) > 0:
        ...
        for node_name, runs in nodes.items():
            if 'z-api' in node_name.lower() or 'http' in node_name.lower():
                print(f"\nNode: {node_name}")
                for idx, run in enumerate(runs):
                    ...
                    if 'data' in run:
                        ...
                        print(f"  > Output Principal [{idx}]:")
                        for item in main_data:
                            for sub_item in item:
                                if 'json' in sub_item:
                                    print(f"      {str(sub_item['json'])[:400]}")
