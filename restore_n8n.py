...
def fix_nodes(nodes):
    for n in nodes:
        if n['name'] == 'Buscar Agendamento':
            n['parameters']['url'] = n['parameters']['url'].replace('$json.body.record.id', '$json.record.id')
        ...
        if n['name'] == 'Preparar':
            for ass in n['parameters']['assignments']['assignments']:
                if ass['name'] == 'ag_id':
                    ass['value'] = ass['value'].replace('json.body.record.id', 'json.record.id')
                    
        if n['name'] == 'Deve Enviar':
            for c in n['parameters']['conditions']['conditions']:
                if c['id'] == 'c1':
                    # Restore original length check, but carefully adapt if Checar Duplicidade outputs empty {}
                    c['leftValue'] = "={{ $('Checar Duplicidade').item.json.id ? 1 : 0 }}"
                    c['rightValue'] = 0
                    c['operator']['type'] = 'number'
                    c['operator']['operation'] = 'equals'
...
try:
    ...
    print("Workflow restorado perfeitamente!", respUpdate.status)
