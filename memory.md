...
## 2026-04-04 — Feature: Preços Variáveis e Ajustes nas Comandas

**O que foi feito:** Adicionado o switch de Valor Variável no cadastro de Serviços e implementado controles de descontos/acréscimos (por valor fixo ou %) na adição de itens da Comanda.

**Arquivos alterados:** `useServicos`, `useAtendimentos`, `Atendimentos.tsx` e `ServicoFormDialog.tsx`.

**Tabelas afetadas:** `servicos` (coluna `valor_variavel`), `atendimento_servicos` e `atendimento_produtos` (campos `tipo_ajuste`, `modo_ajuste`, `valor_ajuste`) modificadas com sucesso e rodando via panel do Supabase.

**Fixes Extras:** Atualizado algoritmo de `useWhatsAppLogs` para injetar a data real do agendamento nos logs virtuais e criada regra cronológica que fixa notificações Pendentes no TOPO ordenadas pela data mais próxima, além de corrigidos erros de PGRST205 (Tabelas e RLS faltantes do módulo Comunicação no Supabase).

---

## 2026-04-25 — Bugfix: Comissão não respeitava desconto da comanda

**Problema:** Quando uma comanda tinha desconto aplicado (global ou por item), a comissão do profissional era calculada sobre o valor **bruto** do serviço, ignorando o desconto. Exemplo: serviço R$120, desconto R$10 → comissão calculada sobre R$120 em vez de R$110.

**Causa raiz:** `useGerarComissao.ts` recebia `item.valor` sem desconto aplicado. Os callers (`CaixaComandas.tsx` e `FecharComandaModal.tsx`) passavam o subtotal bruto do item.

**Correção no código (commit `ed11fe8`):**
- `src/pages/CaixaComandas.tsx` — calcula `fatorDesconto = valor_final / subtotal_bruto` e multiplica cada item antes de passar para `gerarComissoesDaComanda`
- `src/components/atendimentos/FecharComandaModal.tsx` — mesma lógica: `fatorDesconto = atendimento.valor_final / subtotalBruto`

**Correção manual no banco (SQL):**
- Juscelene (Comanda #131): `valor_comissao` R$60 → R$55 (Maicon dos Santos, 50%)
- Sueli da Silva Cruz (Comanda #121): `valor_comissao` R$55 → R$50 (Maicon dos Santos, 50%)
- Marisa Macedo (Comanda #118): `valor_comissao` R$55 → R$50 (Maicon dos Santos, 50%)

**SQL de correção pontual (para comandas com `atendimentos.subtotal = 0`):**
```sql
UPDATE comissoes_registro cr
SET
  valor_servico = ROUND(cr.valor_servico - (a.desconto * cr.valor_servico / NULLIF(items.total, 0)), 2),
  valor_comissao = ROUND((cr.valor_servico - (a.desconto * cr.valor_servico / NULLIF(items.total, 0))) * cr.percentual / 100, 2)
FROM atendimentos a
JOIN (SELECT atendimento_id, SUM(subtotal) as total FROM atendimento_servicos GROUP BY atendimento_id) items ON items.atendimento_id = a.id
WHERE cr.atendimento_id = a.id
  AND a.numero_comanda IN (121, 118)
  AND a.desconto > 0;
```
